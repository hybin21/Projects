...
public:
    ...
    *** MUTATORS ***
    ****************/
    void push_front(const T & value);   // IN - data
    void push_back(const T & value);    // IN - data
    void pop_front() noexcept;
    void clear();
    void select_sort();
    void insert_sorted(const T & value);    // IN - data
    void insert(const int index,    // IN - index
                const T & value);   // IN - data
...
bool LinkedList<T>::Iterator::operator==(
        const LinkedList<T>::Iterator& right) const
{
...
LinkedList<T>::~LinkedList()
{
    ...
    while(current)
    {
        temp = current->next; // CALC - points the next node
        delete current;
        current = temp;
    }
    head = NULL;
 ...
 *****************************************************************************/
template<typename T>
const T& LinkedList<T>::front() const noexcept
{
    if(head == NULL)
        throw(ListEmpty());
    else
 ...
 *****************************************************************************/
template<typename T>
const T & LinkedList<T>::back() const noexcept
{
    if(head == NULL)
        throw(ListEmpty());
    else
 ...
 *****************************************************************************/
template<typename T>
void LinkedList<T>::pop_front() noexcept
{
//    if(head == NULL)
//        throw(ListEmpty());
//    else
...
/*****************************************************************************
 * :insert(const int index, const T & value, bool & check):
 *  Class : LinkedList
 *
 * ---------------------------------------------------------------------------
 * This method insert nocde in index from the user
 *
 * ---------------------------------------------------------------------------
 * PRE-CONDITION
 *   The following need previously defined values:
 *      ths program inserts a nocde
 * POST-CONDITION
 *      the LinkedList will have a new node
 *
 *****************************************************************************/
template <typename T>
void LinkedList<T>::insert(const int index, // IN - input index
                           const T & value) // IN- data of the node

{
    Node<T> * current = head;   // CALC - current position
    Node<T> * temp = new Node<T>(value);    // CALC - temporary to be added
    Node<T> * nextN;     // CACL - next to the current position
    if(index <= 0)
        push_front(value);
